    ...
    for i in range(num_boxes):
        ...
        if class_name not in allowed_classes:
            ...
            if info:
                ...
                # @freeze_time("2020-11-16 18:55:36")

                # freezer = datetime.datetime.now()
                # freezer = freeze_time("2020-11-16 18:55:36")
                # freezer.start()
                ...
                #     time.sleep(1)
                '''
                # now = datetime.now().replace(year = 2020, month = 11, day = 16, microsecond=0)
                # now_hour = int(now.hour)
                # now_minute = int(now.minute)
                # now_second = int(now.second)
                
                # hour_delta = now_hour - int(hour)
                # minute_delta = now_minute - int(minute) 
                # second_delta = now_second - int(second)
                # print(hour_delta,minute_delta,second_delta)
                # abs_hour_delta = abs(hour_delta)
                # abs_minute_delta = abs(minute_delta)
                # abs_second_delta = abs(second_delta)
                # print(abs_hour_delta,abs_minute_delta,abs_second_delta)
                # ndelta = timedelta(hours = 3, minutes = 10, seconds = 5)
                # or_date = now - ndelta 
                # or_date = datetime.now()
                # or_date = or_date.replace(year = 2020, month = 11, day = 16, hour=23,minute=59,second =30)
                # or_date = or_date.isoformat('')

                cv2.circle(image,(tip_xmin,tip_ymin), 5, (255,0,0), -1)
                # print("Object found: {}, Confidence: {}, TipPointCords (tip_xmin, tip_ymin, tip_xmax, tip_ymax): {}, {}, {}, {} ".format(class_name, now, tip_xmin, tip_ymin, tip_xmax, tip_ymax))
